...
__host__ float convertComponent(int expo, int val)
{
	float v = val / 256.0f;
...
__host__ void workOnRGBE(RGBE* scan, int len, float* cols)
{
	while (len-- > 0) {
...
__host__ bool decrunch(RGBE* scanline, int len, FILE* file)
{
	int  i, j;
...
__host__ bool oldDecrunch(RGBE* scanline, int len, FILE* file)
{
	int i;
	int rshift = 0;
